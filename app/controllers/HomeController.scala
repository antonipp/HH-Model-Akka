...
class HomeController @Inject()()
  ...
  val config = ConfigFactory.parseString("akka.remote.netty.tcp.port=0"). //random port
  ...
  def wsFutureFlow(request: RequestHeader): Future[Flow[JsValue, JsValue, NotUsed]] = {
    ...
    val userActor = createUserActor(webSocketOut)

    // Once we have an actor available, create a flow...
    Future{ createWebSocketFlow(webSocketIn, userActor) }
  }
  ...
  /**
   * Creates a master actor using the websocket out actor for output.
	 *
   * @param webSocketOut the "write" side of the websocket, that the master actor sends JsValue to.
   * @return a master actor for this ws connection.
   */
  def createUserActor(webSocketOut: ActorRef): ActorRef = {
    actorSystem.actorOf(Props(classOf[Master], webSocketOut), name="master")
  }
